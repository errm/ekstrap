...
func (s System) Configure(n *node.Node, cluster *eks.Cluster) error {
	...
	if err := s.writeConfig("/var/lib/kubelet/kubeconfig", kubeconfig, info); err != nil {
		return err
	}

	if err := s.writeConfig("/lib/systemd/system/kubelet.service", kubeletService, info); err != nil {
		return err
	}
